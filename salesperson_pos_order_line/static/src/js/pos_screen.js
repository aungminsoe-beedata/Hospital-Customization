   ...
   async onClick() {
            ...
            const salespersonList = this.pos.res_users.map((salesperson) => {
                    return {
                        id: salesperson.id,
                        item: salesperson,
                        label: salesperson.partner_id[1]    ,
                        isSelected: false,
                        
                    };
                });
              const { confirmed, payload: salesperson } = await this.popup.add(SelectionPopup, {
                title: _t("Select the Physicians"),
                list: salespersonList,
            });
              if (confirmed) {
             this.pos.selectedOrder.selected_orderline.salesperson = salesperson.partner_id;
             this.pos.selectedOrder.selected_orderline.physician_id = salesperson.id;
        }
    }
}
